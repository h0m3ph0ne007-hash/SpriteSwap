  ...
  const ready=(async()=>{
    try{
      ...
      window.SpriteSwapBackend={
        ...
        async syncProfile(){const u=await this.user();if(!u)return;const username=u.user_metadata?.full_name||u.user_metadata?.name||u.email?.split('@')[0]||'Trader';const {error}=await client.from('profiles').upsert({user_id:u.id,username,updated_at:new Date().toISOString()});if(error)console.warn('Profile backend sync failed:',error)}
      };
